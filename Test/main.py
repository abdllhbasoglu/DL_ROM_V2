...
def loss_function(reconstructed, origin, device, alpha=1.0, beta=1.0, mode='log', h_dim=150, w_dim=576):
...
def main():
    ...
    reconstructed_data = torch.zeros(len(train_data),3,150,576).to(device)
    for i in range(3):
    ...
    original_data = torch.zeros(len(train_data),3,150,576).to(device)
    for i in range(3):
        for k in range(len(train_data)):
    ...
    reconstructed_val = torch.zeros(len(val_data),3,150,576).to(device)
    for i in range(3):
