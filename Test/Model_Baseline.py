...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        self.l1 = nn.Linear(64 * 2 * 5, 32)  # first linear layer for encoder part
        self.l2 = nn.Linear(32, 64 * 2 * 5) # second linear layer for decoder part
        
        # Decoder kısmı
        self.decoder = nn.Sequential(
            nn.Unflatten(1, (64, 2, 5)),
            
            #1
            nn.Upsample(size=(4,10), mode='bilinear'),
            self.convTranspose_block(64, 64, 3),
                        
            #2
            nn.Upsample(size=(6,18), mode='bilinear'),
            self.convTranspose_block(64, 64, 3),
                                    
            #3
            nn.Upsample(size=(10,36), mode='bilinear'),
            self.convTranspose_block(64, 64, 3),
                        
            #4
            nn.Upsample(size=(20,72), mode='bilinear'),
            self.convTranspose_block(64, 64, 3),
                 
            #5
            nn.Upsample(size=(38,144), mode='bilinear'),
            self.convTranspose_block(64, 64, 3),
                        
            #6
            nn.Upsample(size=(76,288), mode='bilinear'),
            self.convTranspose_block(64, 64, 3),
                                   
            #7
            nn.Upsample(size=(150,576), mode='bilinear'),
...
#summary(autoencoder, (3, 150, 576), device='cuda')
# Giriş verisi
